...
def get_receipt_points(receipt: dict) -> int:
    points = 0
...
def calculate_item_count_points(items: list[dict]) -> int:
    total_count = len(items)
    points_floored = (total_count // 2) * 5
    return points_floored

# If the trimmed length of the item description is a multiple of 3, multiply the price by 0.2 and round up to the nearest integer. The result is the number of points earned.
def calculate_item_description_points(item: dict) -> int:
    len_count = len(item['shortDescription'].strip())
    if len_count % 3 == 0:
